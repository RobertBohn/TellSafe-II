<?php 
   ...
   if (count($_POST) > 0)
   {
      ...
      $p->company = $c->id;
      if (($p->name == "") || ($p->email == "") || ($p->phone1 == "") || ($p->username == "") || ($p->password == ""))
      {
         $msg = "Name, Email, Primary Phone, Username and Password are required fields.";
      }
      else
      {
         $p->Insert();
...
<TR style="background-color=#f5f5f5"><TD>
  ...
  <tr><td>
     ...
     <tr><td><b>Create New Contact</b></td></tr>
     </table>
  </td></tr>
  </table>
</TD></TR>
...
<TR style="background-color=#f5f5f5"><TD>
  <FORM ACTION="addcontact.php?c=<?php print $c->id ?>&" METHOD=post>
